...
pub fn get_lang_id(lang: &str) -> Option<c_int> {
    ...
    let ret = unsafe { ggml_aio_sys::whisper_lang_id(c_lang.as_ptr()) };
    if ret == -1 {
        None
    } else {
        Some(ret)
...
pub fn get_lang_max_id() -> i32 {
    unsafe { ggml_aio_sys::whisper_lang_max_id() }
}
...
pub fn get_lang_str(id: i32) -> Option<&'static str> {
    let c_buf = unsafe { ggml_aio_sys::whisper_lang_str(id) };
    if c_buf.is_null() {
        None
    } else {
...
pub fn get_lang_str_full(id: i32) -> Option<&'static str> {
    let c_buf = unsafe { ggml_aio_sys::whisper_lang_str_full(id) };
    if c_buf.is_null() {
        None
    } else {
        let c_str = unsafe { CStr::from_ptr(c_buf) };
...
pub unsafe fn set_log_callback(
    ...
    unsafe {
        ggml_aio_sys::whisper_log_set(log_callback, user_data);
    }
}
...
pub fn print_system_info() -> &'static str {
    let c_buf = unsafe { ggml_aio_sys::whisper_print_system_info() };
    let c_str = unsafe { CStr::from_ptr(c_buf) };
    c_str.to_str().unwrap()
}
...
impl Default for SystemInfo {
    fn default() -> Self {
        unsafe {
            Self {
                avx: ggml_aio_sys::ggml_cpu_has_avx() != 0,
                avx2: ggml_aio_sys::ggml_cpu_has_avx2() != 0,
                fma: ggml_aio_sys::ggml_cpu_has_fma() != 0,
                f16c: ggml_aio_sys::ggml_cpu_has_f16c() != 0,
            }
        }
    }
}
