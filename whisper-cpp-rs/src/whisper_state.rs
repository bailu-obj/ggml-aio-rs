...
pub struct WhisperState {
    ...
    ptr: *mut ggml_aio_sys::whisper_state,
}
...
impl Drop for WhisperState {
    fn drop(&mut self) {
        unsafe {
            ggml_aio_sys::whisper_free_state(self.ptr);
        }
    }
}
...
impl WhisperState {
    ...
    pub fn pcm_to_mel(&mut self, pcm: &[f32], threads: usize) -> Result<(), WhisperError> {
        ...
        let ret = unsafe {
            ggml_aio_sys::whisper_pcm_to_mel_with_state(
                self.ctx.ctx,
                self.ptr,
                pcm.as_ptr(),
                pcm.len() as c_int,
    ...
    pub fn set_mel(&mut self, data: &[f32]) -> Result<(), WhisperError> {
        ...
        let ret = unsafe {
            ggml_aio_sys::whisper_set_mel_with_state(
                self.ctx.ctx,
                self.ptr,
                data.as_ptr(),
                n_len as c_int,
    ...
    pub fn encode(&mut self, offset: usize, threads: usize) -> Result<(), WhisperError> {
        ...
        let ret = unsafe {
            ggml_aio_sys::whisper_encode_with_state(
                self.ctx.ctx,
                self.ptr,
                offset as c_int,
                threads as c_int,
    ...
    pub fn decode(
        ...
        let ret = unsafe {
            ggml_aio_sys::whisper_decode_with_state(
                self.ctx.ctx,
                self.ptr,
                tokens.as_ptr(),
                tokens.len() as c_int,
    ...
    pub fn lang_detect(
        ...
        let ret = unsafe {
            ggml_aio_sys::whisper_lang_auto_detect_with_state(
                self.ctx.ctx,
                self.ptr,
                offset_ms as c_int,
                threads as c_int,
    ...
    pub fn get_logits(&self) -> Result<&[f32], WhisperError> {
        let ret = unsafe { ggml_aio_sys::whisper_get_logits_from_state(self.ptr) };
        if ret.is_null() {
    ...
    pub fn n_len(&self) -> Result<c_int, WhisperError> {
        Ok(unsafe { ggml_aio_sys::whisper_n_len_from_state(self.ptr) })
    }
    ...
    pub fn n_vocab(&self) -> c_int {
        unsafe { ggml_aio_sys::whisper_n_vocab(self.ctx.ctx) }
    }
    ...
    pub fn full_n_segments(&self) -> Result<c_int, WhisperError> {
        Ok(unsafe { ggml_aio_sys::whisper_full_n_segments_from_state(self.ptr) })
    }
    ...
    pub fn full_lang_id_from_state(&self) -> Result<c_int, WhisperError> {
        Ok(unsafe { ggml_aio_sys::whisper_full_lang_id_from_state(self.ptr) })
    }
    ...
    pub fn full_get_segment_t0(&self, segment: c_int) -> Result<i64, WhisperError> {
        Ok(unsafe { ggml_aio_sys::whisper_full_get_segment_t0_from_state(self.ptr, segment) })
    }
    ...
    pub fn full_n_tokens(&self, segment: c_int) -> Result<c_int, WhisperError> {
        Ok(unsafe { ggml_aio_sys::whisper_full_n_tokens_from_state(self.ptr, segment) })
    }

    fn full_get_token_raw(&self, segment: c_int, token: c_int) -> Result<&CStr, WhisperError> {
        let ret = unsafe {
            ggml_aio_sys::whisper_full_get_token_text_from_state(
                self.ctx.ctx,
                self.ptr,
                segment,
                token,
    ...
    pub fn full_get_token_id(
        ...
        Ok(unsafe {
            ggml_aio_sys::whisper_full_get_token_id_from_state(self.ptr, segment, token)
        })
    }
    ...
    pub fn full_get_token_data(
        ...
        Ok(unsafe {
            ggml_aio_sys::whisper_full_get_token_data_from_state(self.ptr, segment, token)
        })
    }
    ...
    pub fn full_get_token_prob(&self, segment: c_int, token: c_int) -> Result<f32, WhisperError> {
        Ok(
            unsafe {
                ggml_aio_sys::whisper_full_get_token_p_from_state(self.ptr, segment, token)
            },
        )
    }
    ...
    pub fn full_get_segment_speaker_turn_next(&mut self, i_segment: c_int) -> bool {
        unsafe {
            ggml_aio_sys::whisper_full_get_segment_speaker_turn_next_from_state(
                self.ptr, i_segment,
            )
        }
    }
