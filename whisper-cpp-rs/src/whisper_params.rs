...
impl<'a, 'b> FullParams<'a, 'b> {
    ...
    pub fn new(sampling_strategy: SamplingStrategy) -> FullParams<'a, 'b> {
        let mut fp = unsafe {
            ggml_aio_sys::whisper_full_default_params(match sampling_strategy {
                SamplingStrategy::Greedy { .. } => {
                    ggml_aio_sys::WHISPER_SAMPLING_GREEDY
                }
                SamplingStrategy::BeamSearch { .. } => {
                    ggml_aio_sys::WHISPER_SAMPLING_BEAM_SEARCH
                }
            } as _)
        };
            ...
            F: FnMut(SegmentCallbackData) + 'static,
        {
            unsafe {
                ...
                for i in s0..n_segments {
                    let text = ggml_aio_sys::whisper_full_get_segment_text_from_state(state, i);
                    let text = CStr::from_ptr(text);

                    let t0 = ggml_aio_sys::whisper_full_get_segment_t0_from_state(state, i);
                    let t1 = ggml_aio_sys::whisper_full_get_segment_t1_from_state(state, i);
