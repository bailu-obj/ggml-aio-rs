...
pub fn llama_time_us() -> i64 {
    unsafe { ggml_aio_sys::llama_time_us() }
}
...
pub fn max_devices() -> usize {
    unsafe { ggml_aio_sys::llama_max_devices() }
}
...
pub fn list_llama_ggml_backend_devices() -> Vec<LlamaBackendDevice> {
    ...
    for i in 0..unsafe { ggml_aio_sys::ggml_backend_dev_count() } {
        ...
        let props = unsafe {
            ...
            ggml_aio_sys::ggml_backend_dev_get_props(dev, &raw mut props);
            props
        };
        let name = cstr_to_string(props.name);
        let description = cstr_to_string(props.description);
        let backend = unsafe { ggml_aio_sys::ggml_backend_dev_backend_reg(dev) };
        let backend_name = unsafe { ggml_aio_sys::ggml_backend_reg_name(backend) };
        ...
        let device_type = match props.type_ {
            ggml_aio_sys::GGML_BACKEND_DEVICE_TYPE_CPU => LlamaBackendDeviceType::Cpu,
            ggml_aio_sys::GGML_BACKEND_DEVICE_TYPE_ACCEL => LlamaBackendDeviceType::Accelerator,
            ggml_aio_sys::GGML_BACKEND_DEVICE_TYPE_GPU => LlamaBackendDeviceType::Gpu,
            ggml_aio_sys::GGML_BACKEND_DEVICE_TYPE_IGPU => LlamaBackendDeviceType::IntegratedGpu,
...
extern "C" fn logs_to_trace(
    level: ggml_aio_sys::ggml_log_level,
    ...
    if level == ggml_aio_sys::GGML_LOG_LEVEL_CONT {
...
pub fn send_logs_to_tracing(options: LogOptions) {
    ...
    unsafe {
        ...
        ggml_aio_sys::llama_log_set(Some(logs_to_trace), llama_heap_state as *mut _);
        ggml_aio_sys::ggml_log_set(Some(logs_to_trace), ggml_heap_state as *mut _);
    }
}
