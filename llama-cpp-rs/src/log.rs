...
fn meta_for_level(
    ...
    match level {
        ggml_aio_sys::GGML_LOG_LEVEL_DEBUG => (&DEBUG_META, &DEBUG_FIELDS),
        ggml_aio_sys::GGML_LOG_LEVEL_INFO => (&INFO_META, &INFO_FIELDS),
        ggml_aio_sys::GGML_LOG_LEVEL_WARN => (&WARN_META, &WARN_FIELDS),
        ggml_aio_sys::GGML_LOG_LEVEL_ERROR => (&ERROR_META, &ERROR_FIELDS),
        _ => {
...
pub(super) struct State {
    ...
    module: Module,
    buffered: std::sync::Mutex<Option<(ggml_aio_sys::ggml_log_level, String)>>,
...
impl State {
    ...
    pub(super) fn cont_buffered_log(&self, text: &str) {
        ...
        if let Some((previous_log_level, mut buffer)) = lock.take() {
            ...
            let level = self
                ...
                as ggml_aio_sys::ggml_log_level;
            tracing::warn!(
    ...
    pub(super) fn buffer_non_cont(&self, level: ggml_aio_sys::ggml_log_level, text: &str) {
        debug_assert!(!text.ends_with('\n'));
        debug_assert_ne!(level, ggml_aio_sys::GGML_LOG_LEVEL_CONT);
    ...
    pub(super) fn emit_non_cont_line(&self, level: ggml_aio_sys::ggml_log_level, text: &str) {
        debug_assert!(text.ends_with('\n'));
        debug_assert_ne!(level, ggml_aio_sys::GGML_LOG_LEVEL_CONT);
        ...
        match level {
            ggml_aio_sys::GGML_LOG_LEVEL_NONE => {
    ...
    pub(super) fn is_enabled_for_level(&self, level: ggml_aio_sys::ggml_log_level) -> bool {
        // CONT logs do not need to check if they are enabled.
        let level = if level == ggml_aio_sys::GGML_LOG_LEVEL_CONT {
            self.previous_level
...
mod tests {
    ...
    fn cont_disabled_log() {
        ...
        logs_to_trace(
            ggml_aio_sys::GGML_LOG_LEVEL_CONT,
            c"world\n".as_ptr(),
            log_ptr,
        );
        ...
        logs_to_trace(ggml_aio_sys::GGML_LOG_LEVEL_CONT, c"\n".as_ptr(), log_ptr);
    }
