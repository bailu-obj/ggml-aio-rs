...
impl LlamaBackend {
    ...
    pub fn init() -> crate::Result<LlamaBackend> {
        Self::mark_init()?;
        unsafe { ggml_aio_sys::llama_backend_init() }
        Ok(LlamaBackend {})
    }
    ...
    pub fn init_numa(strategy: NumaStrategy) -> crate::Result<LlamaBackend> {
        Self::mark_init()?;
        unsafe {
            ggml_aio_sys::llama_numa_init(ggml_aio_sys::ggml_numa_strategy::from(strategy));
        }
        Ok(LlamaBackend {})
    }
    ...
    pub fn supports_mmap(&self) -> bool {
        unsafe { ggml_aio_sys::llama_supports_mmap() }
    }
    ...
    pub fn void_logs(&mut self) {
        ...
        unsafe {
            ggml_aio_sys::llama_log_set(Some(void_log), std::ptr::null_mut());
        }
    }
}
...
pub struct InvalidNumaStrategy(
    ...
    pub ggml_aio_sys::ggml_numa_strategy,
);

impl TryFrom<ggml_aio_sys::ggml_numa_strategy> for NumaStrategy {
    type Error = InvalidNumaStrategy;

    fn try_from(value: ggml_aio_sys::ggml_numa_strategy) -> Result<Self, Self::Error> {
        match value {
            ggml_aio_sys::GGML_NUMA_STRATEGY_DISABLED => Ok(Self::DISABLED),
            ggml_aio_sys::GGML_NUMA_STRATEGY_DISTRIBUTE => Ok(Self::DISTRIBUTE),
            ggml_aio_sys::GGML_NUMA_STRATEGY_ISOLATE => Ok(Self::ISOLATE),
            ggml_aio_sys::GGML_NUMA_STRATEGY_NUMACTL => Ok(Self::NUMACTL),
            ggml_aio_sys::GGML_NUMA_STRATEGY_MIRROR => Ok(Self::MIRROR),
            ggml_aio_sys::GGML_NUMA_STRATEGY_COUNT => Ok(Self::COUNT),
            value => Err(InvalidNumaStrategy(value)),
        }
    }
}

impl From<NumaStrategy> for ggml_aio_sys::ggml_numa_strategy {
    fn from(value: NumaStrategy) -> Self {
        match value {
            NumaStrategy::DISABLED => ggml_aio_sys::GGML_NUMA_STRATEGY_DISABLED,
            NumaStrategy::DISTRIBUTE => ggml_aio_sys::GGML_NUMA_STRATEGY_DISTRIBUTE,
            NumaStrategy::ISOLATE => ggml_aio_sys::GGML_NUMA_STRATEGY_ISOLATE,
            NumaStrategy::NUMACTL => ggml_aio_sys::GGML_NUMA_STRATEGY_NUMACTL,
            NumaStrategy::MIRROR => ggml_aio_sys::GGML_NUMA_STRATEGY_MIRROR,
            NumaStrategy::COUNT => ggml_aio_sys::GGML_NUMA_STRATEGY_COUNT,
        }
    }
}
...
impl Drop for LlamaBackend {
    fn drop(&mut self) {
        ...
        unsafe { ggml_aio_sys::llama_backend_free() }
    }
}
...
mod tests {
    ...
    fn numa_from_and_to() {
        ...
        for numa in &numas {
            let from = ggml_aio_sys::ggml_numa_strategy::from(*numa);
