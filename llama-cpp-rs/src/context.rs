...
pub struct LlamaContext<'a> {
    pub(crate) context: NonNull<ggml_aio_sys::llama_context>,
...
impl<'model> LlamaContext<'model> {
    pub(crate) fn new(
        ...
        llama_context: NonNull<ggml_aio_sys::llama_context>,
        embeddings_enabled: bool,
    ) -> Self {
        Self {
    ...
    /// - If the current model had a pooling type of [`ggml_aio_sys::LLAMA_POOLING_TYPE_NONE`]
    ...
    pub fn embeddings_seq_ith(&self, i: i32) -> Result<&[f32], EmbeddingsError> {
        ...
        unsafe {
            let embedding = ggml_aio_sys::llama_get_embeddings_seq(self.context.as_ptr(), i);
    ...
    pub fn embeddings_ith(&self, i: i32) -> Result<&[f32], EmbeddingsError> {
        ...
        unsafe {
            let embedding = ggml_aio_sys::llama_get_embeddings_ith(self.context.as_ptr(), i);
    ...
    pub fn get_logits(&self) -> &[f32] {
        let data = unsafe { ggml_aio_sys::llama_get_logits(self.context.as_ptr()) };
    ...
    pub fn get_logits_ith(&self, i: i32) -> &[f32] {
        ...
        let data = unsafe { ggml_aio_sys::llama_get_logits_ith(self.context.as_ptr(), i) };
        let len = usize::try_from(self.model.n_vocab()).expect("n_vocab does not fit into a usize");
    ...
    pub fn reset_timings(&mut self) {
        unsafe { ggml_aio_sys::llama_perf_context_reset(self.context.as_ptr()) }
    }

    /// Returns the timings for the context.
    pub fn timings(&mut self) -> LlamaTimings {
        let timings = unsafe { ggml_aio_sys::llama_perf_context(self.context.as_ptr()) };
...
impl Drop for LlamaContext<'_> {
    fn drop(&mut self) {
        unsafe { ggml_aio_sys::llama_free(self.context.as_ptr()) }
    }
}
