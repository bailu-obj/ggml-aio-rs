...
pub struct LlamaModel {
    pub(crate) model: NonNull<ggml_aio_sys::llama_model>,
}
...
pub struct LlamaLoraAdapter {
    pub(crate) lora_adapter: NonNull<ggml_aio_sys::llama_adapter_lora>,
}
...
impl LlamaModel {
    pub(crate) fn vocab_ptr(&self) -> *const ggml_aio_sys::llama_vocab {
        unsafe { ggml_aio_sys::llama_model_get_vocab(self.model.as_ptr()) }
    }
    ...
    pub fn n_ctx_train(&self) -> u32 {
        let n_ctx_train = unsafe { ggml_aio_sys::llama_n_ctx_train(self.model.as_ptr()) };
    ...
    pub fn token_bos(&self) -> LlamaToken {
        let token = unsafe { ggml_aio_sys::llama_token_bos(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn token_eos(&self) -> LlamaToken {
        let token = unsafe { ggml_aio_sys::llama_token_eos(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn token_nl(&self) -> LlamaToken {
        let token = unsafe { ggml_aio_sys::llama_token_nl(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn is_eog_token(&self, token: LlamaToken) -> bool {
        unsafe { ggml_aio_sys::llama_token_is_eog(self.vocab_ptr(), token.0) }
    }
    ...
    pub fn token_sep(&self) -> LlamaToken {
        let token = unsafe { ggml_aio_sys::llama_vocab_sep(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn str_to_token(
        ...
        let size = unsafe {
            ggml_aio_sys::llama_tokenize(
                self.vocab_ptr(),
                c_string.as_ptr(),
                c_int::try_from(c_string.as_bytes().len())?,
                buffer.as_mut_ptr().cast::<ggml_aio_sys::llama_token>(),
                buffer_capacity,
                add_bos,
                true,
            )
    ...
    pub fn token_attr(&self, LlamaToken(id): LlamaToken) -> LlamaTokenAttrs {
        let token_type = unsafe { ggml_aio_sys::llama_token_get_attr(self.vocab_ptr(), id) };
    ...
    pub fn n_vocab(&self) -> i32 {
        unsafe { ggml_aio_sys::llama_n_vocab(self.vocab_ptr()) }
    }
    ...
    pub fn vocab_type(&self) -> VocabType {
        // ggml_aio_sys::llama_model_get_vocab
        let vocab_type = unsafe { ggml_aio_sys::llama_vocab_type(self.vocab_ptr()) };
    ...
    pub fn n_embd(&self) -> c_int {
        unsafe { ggml_aio_sys::llama_n_embd(self.model.as_ptr()) }
    }

    /// Returns the total size of all the tensors in the model in bytes.
    pub fn size(&self) -> u64 {
        unsafe { ggml_aio_sys::llama_model_size(self.model.as_ptr()) }
    }
    ...
    pub fn n_layer(&self) -> u32 {
        ...
        u32::try_from(unsafe { ggml_aio_sys::llama_model_n_layer(self.model.as_ptr()) }).unwrap()
    }
    ...
    pub fn n_head(&self) -> u32 {
        // It's never possible for this to panic because while the API interface is defined as an int32_t,
        // the field it's accessing is a uint32_t.
        u32::try_from(unsafe { ggml_aio_sys::llama_model_n_head(self.model.as_ptr()) }).unwrap()
    }
    ...
    pub fn meta_val_str(&self, key: &str) -> Result<String, MetaValError> {
        ...
        extract_meta_string(
            |buf_ptr, buf_len| unsafe {
                ggml_aio_sys::llama_model_meta_val_str(
                    self.model.as_ptr(),
                    key_ptr,
                    buf_ptr,
                    buf_len,
    ...
    pub fn meta_count(&self) -> i32 {
        unsafe { ggml_aio_sys::llama_model_meta_count(self.model.as_ptr()) }
    }
    ...
    pub fn meta_key_by_index(&self, index: i32) -> Result<String, MetaValError> {
        extract_meta_string(
            |buf_ptr, buf_len| unsafe {
                ggml_aio_sys::llama_model_meta_key_by_index(
                    self.model.as_ptr(),
                    index,
                    buf_ptr,
                    buf_len,
    ...
    pub fn meta_val_str_by_index(&self, index: i32) -> Result<String, MetaValError> {
        extract_meta_string(
            |buf_ptr, buf_len| unsafe {
                ggml_aio_sys::llama_model_meta_val_str_by_index(
                    self.model.as_ptr(),
                    index,
                    buf_ptr,
                    buf_len,
    ...
    pub fn rope_type(&self) -> Option<RopeType> {
        match unsafe { ggml_aio_sys::llama_model_rope_type(self.model.as_ptr()) } {
            ggml_aio_sys::LLAMA_ROPE_TYPE_NONE => None,
            ggml_aio_sys::LLAMA_ROPE_TYPE_NORM => Some(RopeType::Norm),
            ggml_aio_sys::LLAMA_ROPE_TYPE_NEOX => Some(RopeType::NeoX),
            ggml_aio_sys::LLAMA_ROPE_TYPE_MROPE => Some(RopeType::MRope),
            ggml_aio_sys::LLAMA_ROPE_TYPE_VISION => Some(RopeType::Vision),
            rope_type => {
    ...
    pub fn chat_template(
        ...
        let result =
            unsafe { ggml_aio_sys::llama_model_chat_template(self.model.as_ptr(), name_ptr) };
    ...
    pub fn lora_adapter_init(
        ...
        let cstr = CString::new(path)?;
        let adapter =
            unsafe { ggml_aio_sys::llama_adapter_lora_init(self.model.as_ptr(), cstr.as_ptr()) };
    ...
    pub fn new_context<'a>(
        ...
        let context = unsafe {
            ggml_aio_sys::llama_new_context_with_model(self.model.as_ptr(), context_params)
        };
    ...
    pub fn apply_chat_template(
        ...
        // Build our ggml_aio_sys chat messages
        let chat: Vec<ggml_aio_sys::llama_chat_message> = chat
            .iter()
            .map(|c| ggml_aio_sys::llama_chat_message {
                role: c.role.as_ptr(),
        ...
        let res = unsafe {
            ggml_aio_sys::llama_chat_apply_template(
                tmpl_ptr,
                chat.as_ptr(),
                chat.len(),
                add_ass,
...
impl Drop for LlamaModel {
    fn drop(&mut self) {
        unsafe { ggml_aio_sys::llama_free_model(self.model.as_ptr()) }
    }
}
...
pub enum VocabType {
    /// Byte Pair Encoding
    BPE = ggml_aio_sys::LLAMA_VOCAB_TYPE_BPE as _,
    /// Sentence Piece Tokenizer
    SPM = ggml_aio_sys::LLAMA_VOCAB_TYPE_SPM as _,
}
...
pub enum LlamaTokenTypeFromIntError {
    ...
    UnknownValue(ggml_aio_sys::llama_vocab_type),
}

impl TryFrom<ggml_aio_sys::llama_vocab_type> for VocabType {
    type Error = LlamaTokenTypeFromIntError;

    fn try_from(value: ggml_aio_sys::llama_vocab_type) -> Result<Self, Self::Error> {
        match value {
            ggml_aio_sys::LLAMA_VOCAB_TYPE_BPE => Ok(VocabType::BPE),
            ggml_aio_sys::LLAMA_VOCAB_TYPE_SPM => Ok(VocabType::SPM),
