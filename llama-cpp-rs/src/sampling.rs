...
pub struct LlamaSampler {
    pub(crate) sampler: *mut ggml_aio_sys::llama_sampler,
}
...
impl LlamaSampler {
    ...
    pub fn accept(&mut self, token: LlamaToken) {
        unsafe { ggml_aio_sys::llama_sampler_accept(self.sampler, token.0) }
    }
    ...
    pub fn accept_many(&mut self, tokens: impl IntoIterator<Item = impl Borrow<LlamaToken>>) {
        for token in tokens {
            unsafe { ggml_aio_sys::llama_sampler_accept(self.sampler, token.borrow().0) }
        }
    }
    ...
    pub fn reset(&mut self) {
        unsafe {
            ggml_aio_sys::llama_sampler_reset(self.sampler);
        }
    }
    ...
    pub fn get_seed(&self) -> u32 {
        unsafe { ggml_aio_sys::llama_sampler_get_seed(self.sampler) }
    }
    ...
    pub fn temp(t: f32) -> Self {
        let sampler = unsafe { ggml_aio_sys::llama_sampler_init_temp(t) };
        Self { sampler }
    }
    ...
    pub fn temp_ext(t: f32, delta: f32, exponent: f32) -> Self {
        let sampler = unsafe { ggml_aio_sys::llama_sampler_init_temp_ext(t, delta, exponent) };
        Self { sampler }
    }
    ...
    pub fn top_k(k: i32) -> Self {
        let sampler = unsafe { ggml_aio_sys::llama_sampler_init_top_k(k) };
        Self { sampler }
    }
    ...
    pub fn top_n_sigma(n: f32) -> Self {
        let sampler = unsafe { ggml_aio_sys::llama_sampler_init_top_n_sigma(n) };
        Self { sampler }
    }
    ...
    pub fn typical(p: f32, min_keep: usize) -> Self {
        let sampler = unsafe { ggml_aio_sys::llama_sampler_init_typical(p, min_keep) };
        Self { sampler }
    }
    ...
    pub fn top_p(p: f32, min_keep: usize) -> Self {
        let sampler = unsafe { ggml_aio_sys::llama_sampler_init_top_p(p, min_keep) };
        Self { sampler }
    }
    ...
    pub fn min_p(p: f32, min_keep: usize) -> Self {
        let sampler = unsafe { ggml_aio_sys::llama_sampler_init_min_p(p, min_keep) };
        Self { sampler }
    }
    ...
    pub fn xtc(p: f32, t: f32, min_keep: usize, seed: u32) -> Self {
        let sampler = unsafe { ggml_aio_sys::llama_sampler_init_xtc(p, t, min_keep, seed) };
        Self { sampler }
    }
    ...
    pub fn grammar(
        ...
        let sampler = unsafe {
            ggml_aio_sys::llama_sampler_init_grammar(
                model.vocab_ptr(),
                grammar_str.as_ptr(),
                grammar_root.as_ptr(),
            )
    ...
    pub fn grammar_lazy(
        ...
        let sampler = unsafe {
            ggml_aio_sys::llama_sampler_init_grammar_lazy(
                model.vocab_ptr(),
                grammar_str.as_ptr(),
                grammar_root.as_ptr(),
    ...
    pub fn dry(
        ...
        let sampler = unsafe {
            ggml_aio_sys::llama_sampler_init_dry(
                model.vocab_ptr(),
                model
                    .n_ctx_train()
                    .try_into()
    ...
    pub fn penalties(
        ...
        let sampler = unsafe {
            ggml_aio_sys::llama_sampler_init_penalties(
                penalty_last_n,
                penalty_repeat,
                penalty_freq,
                penalty_present,
    ...
    pub fn mirostat(n_vocab: i32, seed: u32, tau: f32, eta: f32, m: i32) -> Self {
        let sampler =
            unsafe { ggml_aio_sys::llama_sampler_init_mirostat(n_vocab, seed, tau, eta, m) };
        Self { sampler }
    }
    ...
    pub fn mirostat_v2(seed: u32, tau: f32, eta: f32) -> Self {
        let sampler = unsafe { ggml_aio_sys::llama_sampler_init_mirostat_v2(seed, tau, eta) };
        Self { sampler }
    }
    ...
    pub fn dist(seed: u32) -> Self {
        let sampler = unsafe { ggml_aio_sys::llama_sampler_init_dist(seed) };
        Self { sampler }
    }
    ...
    #[must_use]
    pub fn greedy() -> Self {
        let sampler = unsafe { ggml_aio_sys::llama_sampler_init_greedy() };
        Self { sampler }
    }
    ...
    pub fn logit_bias(n_vocab: i32, biases: &[LlamaLogitBias]) -> Self {
        let data = biases.as_ptr().cast::<ggml_aio_sys::llama_logit_bias>();

        let sampler = unsafe {
            ggml_aio_sys::llama_sampler_init_logit_bias(n_vocab, biases.len() as i32, data)
        };
...
impl Drop for LlamaSampler {
    fn drop(&mut self) {
        unsafe {
            ggml_aio_sys::llama_sampler_free(self.sampler);
        }
    }
}
