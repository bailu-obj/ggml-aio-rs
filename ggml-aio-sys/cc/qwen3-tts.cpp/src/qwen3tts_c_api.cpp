...
// Last load_models error when qwen3_tts_create fails (handle is destroyed).
static std::string g_qwen3_tts_init_error;
...
Qwen3Tts * qwen3_tts_create(const char * model_dir, int32_t n_threads) {
    ...
    if (!tts->engine.load_models(model_dir)) {
        g_qwen3_tts_init_error = tts->engine.get_error();
        tts->last_error = g_qwen3_tts_init_error;
        delete tts;
        return nullptr;
    }
    return tts;
...
const char * qwen3_tts_last_init_error(void) {
    return g_qwen3_tts_init_error.c_str();
}
